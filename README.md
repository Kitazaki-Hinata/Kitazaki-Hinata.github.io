...
| 快乐的事 | `/happy/` | `src/pages/happy/index.astro` | `resource/happy/<id>/post.json` 和可选图片 |
...
绘画展示会自动按图片宽高和连续作品布局，无需额外配置。作品在页面中按新到旧显示，宽度大于或等于高度的图片（含方图）按横图处理，三栏布局规则如下：

- 单张横图占两栏，竖图占一栏。
- 恰好连续两张横图：较新的横图默认独占三栏；较旧的横图占下一行左侧两栏，可与紧邻的更旧竖图（右侧一栏）同排。例如 No.6、No.5 为横图而 No.4 为竖图时，No.6 独占第一行，下一行为 No.5 在左、No.4 在右。
- 若这两张横图前还有紧邻的更新竖图，则该竖图占左侧一栏、较新的横图占右侧两栏。例如新增竖图 No.7 后，第一行为 No.7 在左、No.6 在右，下一行仍为 No.5 在左、No.4 在右。
- 连续三张及以上横图：这一组的每张横图都独占一行三栏。

展示区域不足三栏时，中等宽度改为两栏（横图占两栏、竖图占一栏）；手机等窄屏按作品原顺序逐张单列展示。图片保持原始比例，不裁剪，原图查看器也沿用作品原顺序。
...
访问 `/happy/`，按时间浏览生活短文，照片可选。每条记录可以只写文字，也可以配一张或多张图片，文字只显示一次；纯文字记录不显示图片区或空图片提示。点击图片打开原图查看器，上一张／下一张只切换这一条记录的图片。栏目使用内页背景，并复用缩略图和查看器。

每条记录使用 `resource/happy/` 下的一个独立文件夹。只需创建 `post.json`，需要配图时再添加 `images/` 文件夹：
...
│       ├── 01-photo.jpg
│       └── 02-photo.jpg
└── 2026-09-12-a-small-joy/
    └── post.json
```

文件夹名使用小写英文、数字和短横线，建议以日期开头。最简 `post.json` 只需填写 `text`，无需创建图片文件夹：

```json
{
  "text": "今天遇到了一件开心的小事，想把它记下来。"
}
```

需要配图时，把照片放进这一条记录的 `images/` 文件夹即可，系统会自动读取，支持子目录。带可选字段的示例：
...
| `title` | 可选；省略时展示短文、可用日期及可选图片 |
| `date` | 可选的真实 `YYYY-MM-DD` 日期；省略时取文件夹名开头的日期，仍没有则隐藏日期 |
| `alt` | 可选的图片替代文字；省略时使用记录标题与图片序号 |
| `draft` | 默认 false；true 时不展示该记录，也不发布其图片 |
| `images` | 可选数组；省略时按文件名自然排序读取 `images/` 内的全部图片，没有图片也可发布；填写时只发布数组中指定的图片并遵循数组顺序；设为 `[]` 时不展示或发布该记录的图片 |
...
这些路径相对于当前记录文件夹，必须指向其 `images/` 内的图片，大小写一致，不可重复引用。已有照片但暂时只想展示文字时，可设置 `"images": []`，无需删除原图。缺少 `post.json`、短文为空、无效日期、显式指定的图片不存在或引用其他记录的图片都会报告错误。草稿可以先只填写 `{"draft": true}`，等内容准备好后再发布。

记录按日期倒序排列，无日期的在后，同日期按文件夹名称排序。图文直接展示在栏目列表中；使用 `/happy/#happy-2026-09-10-example` 这样的锚点地址可定位到某条记录，重命名文件夹会改变锚点。

添加时新建记录文件夹和 `post.json`，按需放入照片；更新时修改 `post.json` 或替换原图；删除时移除整条记录的文件夹。开发预览会自动刷新，删除或隐藏后，生成目录中的对应图片也会清理。没有已发布记录时，页面显示空状态；纯文字记录会正常显示。需要配图时上传原图即可，压缩缩略图由脚本自动生成。

### 找我&投喂

页面固定展示“个人联系方式”和“赞助渠道”两张图片，点击可以打开原图查看器，支持放大、缩小和切换。页面使用配置指定的内页背景。

两张卡片上下排列，各占一整行，图片区采用 3:1 的横幅比例，适合横向排列多个二维码。建议两张原图统一使用 **2400 × 800 像素的 PNG**；其他比例的图片会等比完整显示并留白。手机端同样上下排列，可点击查看原图并放大。
...
如果修改或更新了 `scripts/` 中的资源处理代码（例如图片必填规则），需在运行开发服务的终端按 `Ctrl+C`，再执行 `npm run dev`。开发进程会缓存启动时加载的脚本，仅刷新浏览器不会加载新的校验规则。
